...
public final class JarFilter {
	...
	public static boolean shouldManage(Path jar) {
		ModSyncConfig.Config config = ModSyncConfig.get();
		if (config.filterMode() == ModSyncConfig.FilterMode.NONE) {
			return true;
		}
		String path = relativePath(jar);
		boolean matches = config.filterRules().stream().anyMatch(rule -> matches(rule, path));
		return config.filterMode() == ModSyncConfig.FilterMode.WHITELIST ? matches : !matches;
	}
