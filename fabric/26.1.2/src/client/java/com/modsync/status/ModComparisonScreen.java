...
public final class ModComparisonScreen extends Screen {
	...
	private ModComparisonScreen(Screen parent, ServerModInfo serverInfo, String serverAddress, List<Comparison> comparisons) {
		super(Component.translatable("modsync.mods.title"));
		this.parent = parent;
	...
	protected void init() {
		...
		this.necessarySyncButton = this.addRenderableWidget(Button.builder(Component.translatable("modsync.mods.sync_necessary"), button -> this.syncNecessary())
			.bounds(this.width / 2 - 200, this.height - 60, 196, 20)
			.build());
		this.fullSyncButton = this.addRenderableWidget(Button.builder(Component.translatable("modsync.mods.full_sync"), button -> this.fullSync())
			.bounds(this.width / 2 + 4, this.height - 60, 196, 20)
			.build());
		this.backButton = this.addRenderableWidget(Button.builder(Component.translatable("modsync.mods.back"), button -> this.onClose())
			.bounds(this.width / 2 - 100, this.height - 36, 200, 20)
			.build());
		this.closeGameButton = this.addRenderableWidget(Button.builder(Component.translatable("modsync.mods.close_game"), button -> this.minecraft.stop())
	...
	public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
		...
		graphics.centeredText(
			this.font,
			this.syncStatus.getString().isBlank() ? Component.translatable("modsync.mods.subtitle", matched, this.comparisons.size()) : this.syncStatus,
			this.width / 2,
			34,
	...
	private void runSync(Function<Consumer<ModSyncService.Result>, java.util.concurrent.CompletableFuture<ModSyncService.Result>> sync, boolean includeClientOnlyMissing) {
		...
		sync.apply(progress).exceptionally(throwable -> {
			...
			this.syncStatus = result.success()
				? Component.translatable("modsync.mods.sync_done", result.completed(), result.total())
				: syncFailed(result);
			if (result.success()) {
	...
	private void renderProgress(GuiGraphicsExtractor graphics) {
		...
		graphics.centeredText(
			this.font,
			Component.translatable("modsync.mods.progress", this.progressCompleted, this.progressTotal, Math.round(progress * 100.0F)),
			this.width / 2,
			top,
			0xFFFFFFFF
		);
		...
		private static final class Entry extends ObjectSelectionList.Entry<Entry> {
			...
			public void extractContent(GuiGraphicsExtractor graphics, int mouseX, int mouseY, boolean hovered, float delta) {
				...
				if (this.comparison.matches()) {
					versionText = Component.translatable("modsync.mods.server_version", serverMod.version());
					status = Component.translatable("modsync.mods.installed");
				} else if (this.comparison.clientOnlyMissing()) {
					versionText = Component.translatable("modsync.mods.server_version", serverMod.version());
					status = Component.translatable("modsync.mods.client_only");
				} else if (this.comparison.downloaded()) {
					versionText = Component.translatable("modsync.mods.server_version", serverMod.version());
					status = Component.translatable("modsync.mods.downloaded");
					color = MATCH_COLOR;
				} else if (this.comparison.state == DownloadState.DOWNLOADING) {
					versionText = Component.translatable("modsync.mods.server_version", serverMod.version());
					status = Component.translatable("modsync.mods.downloading");
				} else if (this.comparison.state == DownloadState.FAILED) {
					versionText = Component.translatable("modsync.mods.server_version", serverMod.version());
					status = Component.translatable("modsync.mods.download_failed");
				} else if (this.comparison.modrinthState == ModrinthState.CHECKING) {
					versionText = Component.translatable("modsync.mods.server_version", serverMod.version());
					status = Component.translatable("modsync.mods.checking_modrinth");
				} else if (this.comparison.modrinthState == ModrinthState.UNAVAILABLE) {
					versionText = Component.translatable("modsync.mods.server_version", serverMod.version());
					status = Component.translatable("modsync.mods.not_on_modrinth");
				} else if (this.comparison.localMod() == null) {
					versionText = Component.translatable("modsync.mods.server_version", serverMod.version());
					status = Component.translatable("modsync.mods.missing");
				} else {
					versionText = Component.translatable("modsync.mods.version_comparison", serverMod.version(), this.comparison.localMod().version());
					status = Component.translatable("modsync.mods.wrong_version");
				}
			...
			private String statusKey() {
				if (this.comparison.matches()) {
					return "modsync.mods.installed";
				}
				if (this.comparison.clientOnlyMissing()) {
					return "modsync.mods.client_only";
				}
				return this.comparison.localMod() == null ? "modsync.mods.missing" : "modsync.mods.wrong_version";
			}
		}
	}
}
