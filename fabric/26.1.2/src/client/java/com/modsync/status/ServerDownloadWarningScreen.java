...
final class ServerDownloadWarningScreen extends Screen {
	...
	ServerDownloadWarningScreen(Screen parent, String serverAddress, List<ModEntry> mods, Runnable trustAndDownload, Runnable cancel) {
		super(Component.translatable("modsync.warning.title"));
		this.parent = parent;
	...
	protected void init() {
		...
		this.addRenderableWidget(Button.builder(Component.translatable("modsync.warning.back"), button -> this.onClose())
			.bounds(this.width / 2 - 204, this.height - 32, 200, 20)
			.build());
		this.addRenderableWidget(Button.builder(Component.translatable("modsync.warning.trust_download"), button -> {
	...
	public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
		...
		graphics.centeredText(this.font, Component.translatable("modsync.warning.security_risk"), this.width / 2, 31, 0xFFFFAA00);
		graphics.centeredText(this.font, Component.translatable("modsync.warning.only_if_trusted"), this.width / 2, 44, 0xFFFFFFFF);
		graphics.centeredText(this.font, Component.translatable("modsync.warning.direct_files", this.mods.size(), this.serverAddress), this.width / 2, 61, 0xFFAAAAAA);
	}
	...
	private static String formatSize(long bytes) {
		if (bytes < 0) {
			return Component.translatable("modsync.warning.unknown_size").getString();
		}
		if (bytes < 1024) {
			return bytes + " B";
		}
		...
		private static final class Entry extends ObjectSelectionList.Entry<Entry> {
			...
			public void extractContent(GuiGraphicsExtractor graphics, int mouseX, int mouseY, boolean hovered, float delta) {
				...
				String authors = this.mod.authors().isEmpty()
					? Component.translatable("modsync.warning.unknown_author").getString()
				...
				graphics.text(minecraft.font, Component.translatable("modsync.warning.metadata", this.mod.version(), authors, this.mod.environment()), this.getContentX() + 4, this.getContentY() + 15, 0xFFCCCCCC);
				graphics.text(minecraft.font, Component.translatable("modsync.warning.file_metadata", filename, formatSize(this.mod.fileSize())), this.getContentX() + 4, this.getContentY() + 27, 0xFFAAAAAA);
			}
