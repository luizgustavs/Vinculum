...
public final class ServerInfoRenderer {
	...
	public static void render(GuiGraphicsExtractor graphics, int x, int y, int width, int mouseX, int mouseY, ServerModInfo info) {
		...
		Component version = Component.translatable("modsync.server.version", info.fabricVersion());
		Component modCount = Component.translatable("modsync.server.mod_count", info.mods().size());
		int right = x + width;
		...
		if (mouseX >= left && mouseX <= right + 2 && mouseY >= y && mouseY <= y + 20) {
			...
			tooltip.add(Component.translatable("modsync.server.minecraft", info.minecraftVersion()));
			tooltip.add(Component.translatable("modsync.server.fabric", info.fabricVersion()));
			tooltip.add(Component.translatable("modsync.server.mods", info.mods().size()));
